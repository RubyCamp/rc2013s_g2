...
class Game
  ...
  def initialize(mode)
    ...
    @hairetu =@kimeuchi.loadBulletFile(mode)
    @resimg = Array.new(18).map! {Array.new(20,0)}
    18.times do |y|
      20.times do |x|
        if @hairetu[y][x] == 1
          #num = @kimeuchi.getBullet
          setSelectedBulletImage(mode)
          @selectedBullet = mode
          @bullets << Bullet.new(x*25+275,y*25+75,@selectedBullet_img,mode,@stage,true)
          @resimg[y][x] = @bullets.last
        elsif @hairetu[y][x] == 9
          @resimg[y][x] = 9
        end
      end
  ...
  def play
    ...
    #   20.times do |x|
    #     if @hairetu[y][x] == 1
    #       @ini << Sprite.new(x*25+290,y*25+80,@resimg[y][x])
    ...
    if Input.mousePush?(M_LBUTTON)
      ...
      if @pointer.x >= 250 
       ...
       position_y = ( @daiza.direction != :up ? @daiza.y + @daiza.rimage.height : @daiza.y)
        bullet = Bullet.new(position_x, position_y,@selectedBullet_img,@selectedBullet,@stage,false)
   ...
   #残り時間のタイトル
    ...
    #ステージクリア判定
    if @stage.bullets.compact.empty?
      if @mode == 1
    ...
    #ゲームオーバー判定
    if @cdTimer.min == '00' && @cdTimer.sec <= '00'
